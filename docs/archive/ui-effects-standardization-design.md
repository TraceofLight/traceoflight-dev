...
4. 입력/셀렉트/필드 래퍼와 modal/sheet/popover surface를 역할 기준으로 재분류
5. 소스 기반 테스트를 raw 클래스 문자열에서 공용 효과 사용 기준으로 전환
