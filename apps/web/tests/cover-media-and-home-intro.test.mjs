...
test("series cards and sidebars route db cover images through browser-sized urls", async () => {
  ...
  assert.match(seriesCardSource, /data-fallback-src=\{normalizedFallbackCoverImageSrc\}/);
  ...
  assert.match(seriesDetailSource, /data-fallback-src=\{seriesCoverFallbackSrc\}/);
  assert.match(seriesDetailSource, /data-fallback-src=\{seriesPostCoverFallbackSrc\}/);
  ...
  assert.match(blogPostLayoutSource, /data-fallback-src=\{seriesPostFallbackSrc\}/);
});
