...
test("content and db sources both expose cover media through the shared type", async () => {
  ...
  assert.match(blogDbSource, /import \{ normalizeCoverMedia, type CoverMedia \} from '\.\/cover-media';/);
  assert.match(blogDbSource, /coverMedia\?: CoverMedia;/);
  assert.match(blogDbSource, /coverMedia:\s*normalizeCoverMedia\(post\.cover_image_url\)/);
});
