...
const adminIndexPath = new URL('../src/pages/admin/index.astro', import.meta.url);
...
test('admin dashboard can render optional GA4 reports link', async () => {
  const source = await readFile(adminIndexPath, 'utf8');

  assert.match(source, /GA4_REPORTS_URL/);
