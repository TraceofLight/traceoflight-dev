...
test("footer icons use the same filled pill treatment as the admin entry button", async () => {
  ...
  assert.match(footerAdminModalSource, /size="icon"/);
  assert.match(footerAdminModalSource, /variant="outline"/);
});
...
test("home sections and content cards use solid white surfaces with stronger hover lift", async () => {
  ...
  assert.match(
    projectCardSource,
    /bg-white\/92 text-card-foreground shadow-\[0_24px_60px_rgba\(15,23,42,0\.08\)\] transition duration-300 hover:-translate-y-1\.5 hover:bg-white hover:shadow-\[0_32px_80px_rgba\(15,23,42,0\.12\)\]/,
  );
  assert.match(
    postCardSource,
  ...
  assert.match(postCardSource, /aspect-\[4\/5\]/);
  assert.match(postCardSource, /min-h-\[18rem\]/);
  assert.match(postCardSource, /sm:min-h-\[22rem\]/);
  assert.match(postCardSource, /object-cover object-top/);
});
