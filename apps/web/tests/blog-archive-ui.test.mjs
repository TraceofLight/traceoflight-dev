...
test("blog archive page provides search, sort, and visibility filters", async () => {
  ...
  assert.doesNotMatch(source, /id="blog-import-build"/);
  assert.doesNotMatch(source, /id="blog-import-apply"/);
  assert.doesNotMatch(source, /id="blog-import-username"/);
  assert.doesNotMatch(source, /id="blog-import-snapshot-id"/);
...
test("blog archive page includes client script for filtering and sorting cards", async () => {
  ...
  assert.doesNotMatch(source, /traceoflight\.velog\.import\.snapshot-id/);
  assert.doesNotMatch(source, /\/internal-api\/imports\/snapshots\/velog/);
  assert.match(
    source,
...
test("archive sort select uses aligned padding with custom arrow placement", async () => {
  ...
  assert.match(
    styleSource,
    /\.blog-archive-control-actions select \{[\s\S]*appearance:\s*none;/,
  );
  assert.match(
    styleSource,
    /\.blog-archive-control-actions select \{[\s\S]*background-position:\s*right 0\.9rem center;/,
  );
});
...
test("hidden archive cards are forced out of layout", async () => {
  ...
  assert.match(
    styleSource,
    /\.blog-archive-grid \.post-card\[hidden\]\s*\{[\s\S]*display:\s*none\s*!important;/,
  );
});
