...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  if (!refreshToken) {
    ...
    return new Response(
      JSON.stringify({ detail: "Refresh token is missing" }),
      {
        status: 401,
  ...
  if (rotation.kind === "stale") {
    serverLogger.warn("admin.refresh_failed", { reason: rotation.kind });
    return new Response(
      JSON.stringify({ detail: "Refresh token is stale", code: "RTR_STALE" }),
      {
        status: 409,
        headers: { "content-type": "application/json" },
      },
  ...
  return new Response(
    JSON.stringify({
      detail:
        "Refresh token is invalid or its credential revision is no longer active",
      code: "RTR_INVALID",
    }),
    {
      status: 401,
