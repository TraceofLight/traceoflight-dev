...
def test_openapi_exposes_series_endpoints_and_metadata() -> None:
    ...
    assert "/api/v1/series" in schema["paths"]
    assert "/api/v1/series/{slug}" in schema["paths"]
    assert "/api/v1/series/{slug}/posts" in schema["paths"]

    list_op = schema["paths"]["/api/v1/series"]["get"]
    create_op = schema["paths"]["/api/v1/series"]["post"]
    reorder_op = schema["paths"]["/api/v1/series/{slug}/posts"]["put"]
